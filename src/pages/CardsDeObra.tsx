...
// Hooks customizados refatorados
...
const CardsDeObra: React.FC = () => {
  ...
  // Hooks refatorados - substitui useState e funções complexas
  const { 
    cards,
    loading: cardsLoading,
    selectedCard,
    setSelectedCard,
    transferirVerba: hookTransferirVerba,
    aprovarCard: hookAprovarCard,
    rejeitarCard: hookRejeitarCard,
    carregarCards
  } = useCardsDeObra();
  ...
  // Estados locais (UI apenas - não duplica cards)
  ...
  const [confirmDeleteCard, setConfirmDeleteCard] = useState<{ isOpen: boolean; cardId: string | null }>({ isOpen: false, cardId: null });
  ...
  const excluirCard = (cardId: string, e: React.MouseEvent) => {
    e.stopPropagation();
    setConfirmDeleteCard({ isOpen: true, cardId });
  };

  const confirmarExclusaoCard = async () => {
    const cardId = confirmDeleteCard.cardId;
    if (!cardId) return;
    ...
    try {
      ...
      carregarCards();
      setConfirmDeleteCard({ isOpen: false, cardId: null });
    } catch (error) {
  ...
  const cancelarExclusaoCard = () => {
    setConfirmDeleteCard({ isOpen: false, cardId: null });
  };

  // Detalhes do card (mantém local - UI específica)
  ...
  // ADMIN: Transferir verba para card (REFATORADO - USA HOOK)
  const transferirVerba = async () => {
    try {
      ...
      // Usa o hook refatorado que encapsula toda a lógica
      await hookTransferirVerba(selectedCard, valorTransf);
      ...
      if (selectedCard) {
        ...
        // NÃO incrementa total_gasto - a verba só conta como gasto na página Obras
  ...
  // ADMIN: Aprovar obra finalizada (REFATORADO - USA HOOK)
  const aprovarObra = async () => {
    try {
      ...
      // Usa o hook refatorado que encapsula toda a lógica de estorno
      await hookAprovarCard(selectedCard);
  ...
  // ADMIN: Rejeitar obra (REFATORADO - USA HOOK)
  const rejeitarObra = async () => {
    try {
      ...
      // Usa o hook refatorado
      await hookRejeitarCard(selectedCard.id_card, motivo);
  ...
  return (
    <div className="cards-obra-page">
          ...
          <div className="modal-detalhes-card" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-body-detalhes">
              ...
              <div className="resumo-financeiro">
                ...
                <div className={`resumo-item ${selectedCard.saldo_atual === 0 ? 'saldo-zero' : ''}`}>
                  <DollarSign size={20} />
                  <div>
                    ...
                    {selectedCard.saldo_atual === 0 && (
                      <span className="aviso-sem-verba">⚠️ Aguardando verba</span>
                    )}
                  </div>
                </div>
                <div className="resumo-item">
                ...
                {!isAdmin && selectedCard.status === 'EM_ANDAMENTO' && (
                  <>
                    <button
                      ...
                      onClick={() => {
                        if (selectedCard.saldo_atual === 0) {
                          toast.error('Aguardando envio de verba para iniciar obra');
                          return;
                        }
                        setShowNovaDespesa(true);
                      }}
                      disabled={selectedCard.saldo_atual === 0}
                      title={selectedCard.saldo_atual === 0 ? 'Sem verba disponível' : 'Registrar despesa'}
                    >
      ...
      <ConfirmDialog
        isOpen={confirmDeleteCard.isOpen}
        title="Confirmar Exclusão do Card"
        message="Tem certeza que deseja excluir este card? Todas as despesas e solicitações de verba associadas também serão excluídas. Esta ação não pode ser desfeita."
        onConfirm={confirmarExclusaoCard}
        onCancel={cancelarExclusaoCard}
